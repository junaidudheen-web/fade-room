...
const DEFAULT_WORDS = ["FADE ROOM", "Precision Grooming", "Modern Style"]
...
export function ParticleTextEffect({ words = DEFAULT_WORDS }: ParticleTextEffectProps) {
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    // Calculate optimal responsive font size
    let fontSize = 120;
    ...
    if (fontSize > canvas.height * 0.4) {
      fontSize = Math.floor(canvas.height * 0.4);
    }
    fontSize = Math.max(24, Math.min(150, fontSize));
    ...
    // Generate new color
    const newColor = {
      r: Math.random() * 100 + 155, // Shifted to brighter colors like gold/silver
  ...
  const animate = () => {
    ...
    // Background with motion blur - transparent so we can see hero background
  ...
  useEffect(() => {
    ...
    // Make canvas responsive to parent
    const resizeCanvas = () => {
  ...
  return (
    <div className="absolute inset-0 w-full h-full z-0 overflow-hidden mix-blend-screen opacity-60">
      <canvas
        ref={canvasRef}
